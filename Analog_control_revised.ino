...
ISR(TIMER1_OVF_vect)        // interrupt service routine for maintaining blink rate.
{
  ...
  for (int i = 0; i < 4; i++)
  {
    if (color_settings[color][i] && color != 5 && color != 6)
    {
      if (LED_status[i])
      {
    ...
    else if (color_settings[color][i] && color == 5)
    {
      if (LED_status[i])
      {
        analogWrite(led_pins[i], 0);
        ...
        analogWrite(led_pins[1], brightness*.75);
        analogWrite(led_pins[2], brightness);
        LED_status[i] = !LED_status[i];
      }

    }
    else if (color_settings[color][i] && color == 6)
    {
      if (LED_status[i])
      {
        analogWrite(led_pins[i], 0);
      ...
      {
        analogWrite(led_pins[0], brightness*.75);
        analogWrite(led_pins[2], brightness);
        LED_status[i] = !LED_status[i];
      }
